...
modify(
  ...
  `className={\`px-4 py-2 bg-[#0b0b14]/90 backdrop-blur-md border flex items-center justify-center gap-2 rounded-xl shadow-lg cursor-pointer transition-all active:scale-95 backdrop-blur-md \${
                chatOpen ? 'border-green-500/50 text-green-400 bg-green-500/10 ring-1 ring-inset ring-green-500/20' : 'border-white/10 hover:border-white/30 text-white hover:bg-white/5 ring-1 ring-inset ring-white/5'
              }\`}`
);
...
modify(
  ...
  `className="px-5 py-4 bg-gradient-to-b from-red-500 to-red-700 hover:from-red-400 hover:to-red-600 text-white rounded-2xl border-2 border-red-400/50 text-center font-bold text-[10px] md:text-xs tracking-widest uppercase shadow-[0_8px_0_#7f1d1d,auto_auto_30px_rgba(220,38,38,0.4)] active:shadow-[0_2px_0_#7f1d1d] active:translate-y-1.5 transition-all w-[120px]"`
);

modify(
  `🚨 REPORT 🚨`,
  `<div className="flex flex-col items-center gap-1"><span className="text-red-200 text-[8px] tracking-[0.2em] mb-0.5">EMERGENCY</span><span>REPORT</span></div>`
);
...
modify(
  ...
  `className={\`py-4 px-6 md:py-4 md:px-8 rounded-2xl border-2 shadow-xl text-center cursor-pointer font-bold uppercase transition-all flex flex-col items-center justify-center relative overflow-hidden w-[140px] \${
                  nearestRoom 
                    ...
                    : 'bg-[#1a1a24] text-slate-500 border-slate-700 cursor-not-allowed opacity-80'
                }\`}`
);
modify(`style={{ minWidth: '110px' }}`, `style={{ minWidth: '140px' }}`);
...
modify(
  ...
  `className="w-full bg-white/5 text-white p-2.5 rounded-lg border border-white/10 focus:outline-none focus:border-[#1a9eff] focus:bg-white/10 transition-colors pr-10 hover:border-white/20"`
);
// chat message box style
