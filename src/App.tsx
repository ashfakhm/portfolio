...
import CrewmateSprite, {
  ...
  CREWMATE_COLORS,
  CREWMATE_HATS,
} from "./components/CrewmateSprite";
...
export default function App() {
  ...
  const [showTaskCompletedBanner, setShowTaskCompletedBanner] = useState<
    string | null
  >(null);
  ...
  const [nearVent, setNearVent] = useState<{
    id: string;
    rx: string;
    x: number;
    y: number;
  ...
  const [ventingStatus, setVentingStatus] = useState<
    "idle" | "diving" | "emerging"
  >("idle");
  ...
  const [joystickCurrent, setJoystickCurrent] = useState<{
    x: number;
    y: number;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    const tickMovement = () => {
      ...
      if (keysActiveRef.current["w"] || keysActiveRef.current["arrowup"])
        dy -= 1;
      if (keysActiveRef.current["s"] || keysActiveRef.current["arrowdown"])
        dy += 1;
      if (keysActiveRef.current["a"] || keysActiveRef.current["arrowleft"])
        dx -= 1;
      if (keysActiveRef.current["d"] || keysActiveRef.current["arrowright"])
        dx += 1;
      ...
      {isMobile && (
        <div className="fixed inset-0 bg-[#07070F] z-[100] flex flex-col items-center justify-center p-8 text-center select-none animate-fadeIn">
          ...
          <p className="text-[10px] text-slate-400 max-w-xs leading-relaxed font-mono uppercase tracking-[0.1em]">
            I am currently working on the mobile responsive layout. For now,
            please use a desktop, laptop, or PC.
          </p>
        </div>
      )}
        ...
        <div className="absolute inset-x-0 inset-y-0 z-10 pointer-events-none flex flex-col justify-between p-3 select-none">
            ...
            <div className="w-full h-3.5 bg-black border border-slate-700 rounded overflow-hidden p-0.5 relative">
              ...
              {progressPercent === 100 ? (
                ...
                >
                  ...
                  <Trophy size={10} className="ml-2 text-yellow-300" />
                </div>
              ) : null}
            </div>
          </div>
          ...
          <div className="absolute left-3 top-20 pointer-events-auto flex flex-col space-y-2.5 max-w-[220px] hidden md:flex">
            ...
            <div className="bg-[#0b0b1ccf] border border-[#3a3a5e] p-2 rounded text-[9px] text-slate-400 font-mono leading-relaxed shadow flex flex-col">
              <span className="font-extrabold uppercase text-[#FFD700] text-[9px] mb-0.5">
                AUTO-WALK CLIENT:
              </span>
              <span>
                Click mission above to auto-pilot. Tap Space / USE to run!
              </span>
            </div>
          </div>
          ...
          <div className="absolute right-3 top-16 sm:right-4 sm:top-24 pointer-events-auto flex flex-col items-end space-y-2 sm:space-y-3">
            ...
            <button
              ...
              className={`hidden md:flex p-2.5 sm:px-4 sm:py-2 bg-[#0b0b14]/90 backdrop-blur-md items-center justify-center gap-2 rounded-xl shadow-lg cursor-pointer transition-all active:scale-95 ${
                chatOpen
                  ? "border-2 border-green-500/50 text-green-400 bg-green-500/10 shadow-[0_0_15px_rgba(34,197,94,0.3)]"
                ...
                <div className="flex gap-1">
                  {["red", "lime", "cyan", "pink", "yellow"].map((col) => (
                    <button
                      ...
                      style={{
                        backgroundColor:
                          CREWMATE_COLORS[col as CrewmateColor].fill,
      ...
      {showVictory && (
        <div className="fixed inset-0 z-[100] bg-black/90 flex flex-col items-center justify-center animate-fadeIn select-none backdrop-blur-md p-4">
          <div
            ...
            style={{
              backgroundImage:
                "radial-gradient(circle at center, #1a9eff 0%, transparent 70%)",
            }}
          ></div>
          <div
            className="text-[#38FEDE] text-5xl md:text-8xl font-black uppercase tracking-[0.1em] drop-shadow-[0_0_40px_rgba(56,254,222,0.8)] mb-8 z-10 text-center w-full max-w-lg"
            style={{
              fontFamily: '"Press Start 2P"',
          ...
          <div className="bg-[#1a1a2e]/80 border-2 border-[#38FEDE] p-4 md:p-6 rounded-lg text-center shadow-[0_0_30px_rgba(56,254,222,0.2)] mb-8 max-w-sm w-full z-10 backdrop-blur-sm relative overflow-hidden">
            ...
            <p className="font-mono text-[#38FEDE] font-bold text-base md:text-lg">
              ALL TASKS COMPLETED
            </p>
