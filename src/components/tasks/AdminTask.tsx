...
export default function AdminTask({ onComplete, isCompleted }: AdminTaskProps) {
	const {
		...
		handleCardDragStart,
		handleCardDrag,
	} = useAdminTask({ onComplete, isCompleted });
					...
					<div className="w-full max-w-[600px] h-full sm:h-auto min-h-[300px] md:min-h-[400px] bg-[#d3d3d3] border-[8px] md:border-[16px] border-[#9e9e9e] rounded-[20px] md:rounded-[30px] p-3 md:p-6 flex flex-col justify-between shadow-2xl relative">
						<div className="flex justify-between items-start gap-4">
							...
							<div className="flex flex-col gap-2 pt-2 pr-2">
								<div
									className={`w-6 h-6 rounded-full border-4 border-slate-600 ${["too-fast", "too-slow", "bad-read"].includes(swipeStatus) ? "bg-red-500 shadow-[0_0_15px_#ef4444]" : "bg-red-900"}`}
								/>
								<div
									className={`w-6 h-6 rounded-full border-4 border-slate-600 ${swipeStatus === "success" ? "bg-green-500 shadow-[0_0_15px_#22c55e]" : "bg-green-900"}`}
								/>
							</div>
						</div>
								...
								>
									<div className="w-full flex-1 flex gap-2 md:gap-3">
										...
										<div className="flex-1 flex flex-col pt-1">
											...
											<span className="text-[10px] font-black text-black uppercase mb-1">
												ASHFAKH M
											</span>
						...
						<div className="md:col-span-2 bg-[#1a1a2e] border-2 border-[#3a3a5e] p-4 rounded-lg flex flex-col justify-between">
							<div className="space-y-4">
								<div className="flex items-center gap-2">
									...
									<h4 className="text-xs text-green-400 font-bold uppercase tracking-wider">
										SECURE AUTHORIZATION
									</h4>
								</div>
								<p className="text-xs text-slate-300 leading-relaxed">
									Credentials authenticated. Ashfakh's technological coordinates
									list is fully exposed across Stations.
								</p>
							</div>

							<div
								className="pt-4 border-t border-[#3a3a5e] space-y-1 text-[10px]"
