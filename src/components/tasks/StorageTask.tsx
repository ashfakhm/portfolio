...
export default function StorageTask({
	onComplete,
	isCompleted,
}: StorageTaskProps) {
	const { fuelLevel, refuelState, startRefueling, stopRefueling } =
		useStorageTask({ onComplete, isCompleted });
				...
				<div className="flex-1 flex flex-col items-center justify-center p-2 gap-6">
					<div className="w-full max-w-sm bg-slate-900 border-4 border-slate-700 rounded-lg p-5 flex flex-col items-center relative overflow-hidden">
						...
						<div className="w-24 h-36 bg-neutral-950 border-4 border-slate-600 rounded-xl relative overflow-hidden flex flex-col justify-end p-1">
							<div className="absolute top-1 left-1.5 text-[8px] font-mono text-zinc-600">
								FUEL CELL
							</div>
							<div
				...
				<div className="flex-1 flex flex-col gap-4 animate-fadeIn">
					<div className="bg-amber-950/20 border-2 border-yellow-500/30 p-3 rounded-lg flex items-center justify-between text-xs text-yellow-500 font-mono">
						<span className="font-bold">
							ESTABLISHED DIRECT CHANNELS TO ASHFAKH'S TRANSMISSIONS
						</span>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						...
						>
							...
							<div>
								<h4 className="text-white font-bold tracking-widest text-sm uppercase font-mono group-hover:text-blue-400 transition-colors">
									LinkedIn Hub
								</h4>
						...
						>
							...
							<div>
								<h4 className="text-white font-bold tracking-widest text-sm uppercase font-mono group-hover:text-slate-300 transition-colors">
									GitHub Repository
								</h4>
								<p className="text-xs text-gray-500 font-mono mt-1">
									@ashfakhm
