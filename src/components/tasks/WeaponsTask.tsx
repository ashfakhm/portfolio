			...
			{!isGameFinished ? (
				<div className="flex-1 flex flex-col items-center justify-center p-1 gap-4">
					...
					<div className="w-full max-w-lg h-[240px] bg-black/60 backdrop-blur-md border border-slate-700/50 rounded-xl relative overflow-hidden select-none cursor-crosshair shadow-inner">
						...
						<div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 size-32 border-2 border-dashed border-[#50F01E]/20 rounded-full pointer-events-none flex items-center justify-center">
							<div className="size-12 border border-[#50F01E]/40 rounded-full" />
						...
						{targets.map((t) => (
							<button
								key={t.id}
								type="button"
								aria-label="Incoming meteor target"
							...
							>
								<div className="size-[4px] bg-zinc-950 rounded-full absolute top-1 left-2 opacity-50" />
								<div className="size-[6px] bg-zinc-950 rounded-full absolute bottom-1.5 right-2 opacity-50" />
								<div className="size-[3px] bg-zinc-950 rounded-full absolute top-3 right-1.5 opacity-50" />
