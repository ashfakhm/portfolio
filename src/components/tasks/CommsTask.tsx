...
export default function CommsTask({ onComplete, isCompleted }: CommsTaskProps) {
	const { downloadProgress, downloadSpeed, downloadState, startDownloading } =
		useCommsTask({ onComplete, isCompleted });
			...
			{downloadState !== "completed" ? (
				<div className="flex-1 flex flex-col items-center justify-center p-2 text-center space-y-6">
					...
					<div className="max-w-md space-y-2">
						<h3
							className="text-[10px] sm:text-xs md:text-sm font-bold text-white uppercase tracking-wider"
							style={{ fontFamily: '"Press Start 2P"' }}
						>
							{downloadState === "downloading"
								? "DOWNLOADING FILES"
								: "INCOMING COMMS FREQUENCY"}
						</h3>
						<p className="text-xs text-gray-400">
							We have intercepted two massive files containing Ashfakh's
							complete production marketplaces, checkout pipelines, and testing
							metrics.
						</p>
					</div>
