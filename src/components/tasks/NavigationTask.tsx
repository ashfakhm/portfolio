...
import { NODES, useNavigationTask } from "./hooks/useNavigationTask";
				...
				<div className="flex-1 flex flex-col items-center justify-center p-1 gap-4">
					...
					>
						...
						<svg className="absolute inset-0 w-full h-full pointer-events-none z-10">
							...
							{currentNodeIndex > 1 && (
								<polyline
									points={NODES.slice(0, currentNodeIndex)
										.map((n) => `${n.x}%,${n.y}%`)
										.join(" ")}
									fill="none"
									stroke="#3b82f6"
						...
						{NODES.map((node, i) => (
							<div
								key={`waypoint-node-${node.x}-${node.y}`}
							...
							>
								{i === NODES.length - 1 && (
									<Target
										className={`size-3 ${i < currentNodeIndex ? "text-white" : "text-gray-500"}`}
									/>
								)}
						...
						>
							<Navigation2 className="size-5 text-white transform rotate-45 -ml-0.5 mt-0.5" />
						</div>
				...
				<div className="flex-1 flex flex-col gap-4 animate-fadeIn">
					<div className="bg-blue-950/20 border-2 border-blue-500/30 p-2.5 rounded-lg flex items-center justify-between text-xs text-blue-400 font-mono">
						<span className="font-bold">
							COURSE LOCK CONFIRMED // RADAR TRACK LOCKED
						</span>
					...
					<div className="bg-[#0b141d] border border-blue-500/20 p-4 rounded-xl space-y-2 text-xs font-mono">
						...
						<p className="text-[11px] text-slate-300 leading-relaxed pt-1">
							Target locked! Ashfakh operates natively from Kozhikode/Malappuram
							hub base inside Kerala (India), delivering remote software code
							globally with precise latency specs.
						</p>
						<div className="bg-[#0a0d14] p-2 rounded text-[10px] text-blue-400 border border-blue-950">
							STATUS: I am currently in Malappuram district and ready to
							relocate if the task or website needs it.
						</div>
					</div>
				</div>
			)}
