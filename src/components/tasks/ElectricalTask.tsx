...
export default function ElectricalTask({
	onComplete,
	isCompleted,
}: ElectricalTaskProps) {
	const {
					...
					<div className="w-full max-w-sm bg-neutral-950/80 backdrop-blur-md border border-slate-700/50 shadow-inner rounded-xl p-5 flex relative min-h-[220px]">
								...
								return (
									<button
										...
										className={`px-3 py-1.5 rounded border-2 font-mono text-[9px] uppercase font-bold text-black transition-all flex items-center gap-2 select-none ${
											isConnected
												? "opacity-40 cursor-not-allowed bg-green-500 border-green-700 text-white"
												: isSelected
													? "ring-2 ring-white scale-105 bg-white text-black"
													: color === "red"
						...
						<div className="absolute inset-x-20 inset-y-0 pointer-events-none z-0">
							<svg className="w-full h-full">
								{Object.entries(wireConnections).map(
									([leftColor, rightColor]) => {
										const leftIdx = ["red", "blue", "yellow", "pink"].indexOf(
											leftColor,
										);
									...
									<button
										...
										className={`px-3 py-1.5 rounded border-2 font-mono text-[9px] uppercase font-bold text-black transition-all flex items-center gap-2 select-none ${
											isMatched
												? "opacity-40 cursor-not-allowed bg-green-500 border-green-700 text-white"
												: activeWireDrag
													? "hover:scale-105 active:scale-95 bg-slate-800 text-white border-dashed border-zinc-500"
													: color === "red"
														? "bg-red-500 border-red-700"
														: color === "blue"
															? "bg-blue-500 border-blue-700 text-white"
				...
				<div className="flex-1 flex flex-col gap-4 animate-fadeIn">
					<div className="bg-lime-950/20 border-2 border-lime-500/30 p-2.5 rounded-lg flex items-center justify-between text-xs text-lime-400 font-mono">
						<span className="font-bold">
							STACK SWITCHBOARD COMPLETED SYSTEM SHUNT
						</span>
					...
					<div className="bg-[#0e160f] border border-lime-500/20 p-4 rounded-xl space-y-3">
						...
						<p className="text-[11px] text-slate-300 leading-relaxed font-mono">
							Connections verified! Just like aligning electrical routes,
							Ashfakh designs fault-tolerant codebases, plugging RESTful
							endpoints cleanly into distributed datastores.
						</p>
						<div className="grid grid-cols-2 gap-3 text-xs pt-1.5">
							<div className="p-2.5 bg-neutral-900 border border-zinc-800 rounded">
								<span className="text-lime-400 block font-bold font-mono text-[10px]">
									ROUTE A/B: FETCH ENDPOINT
								</span>
								<span className="text-zinc-400 text-[10px] block mt-0.5">
									High velocity payload routing under &lt; 80ms latency.
								</span>
							</div>
							<div className="p-2.5 bg-neutral-900 border border-zinc-800 rounded">
								<span className="text-lime-400 block font-bold font-mono text-[10px]">
									SYNC C/D: API GATEWAY
								</span>
								<span className="text-zinc-400 text-[10px] block mt-0.5">
									Dual-auth security matching headers securely.
								</span>
							</div>
						</div>
					</div>
