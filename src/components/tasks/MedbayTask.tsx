...
export default function MedbayTask({
	onComplete,
	isCompleted,
	playerColor,
}: MedbayTaskProps) {
	const { scanProgress, scanDiagnostics, scanState, startScanning } =
		useMedbayTask({ onComplete, isCompleted });
				...
				<div className="flex-1 flex flex-col items-center justify-center p-2 space-y-6">
					<div className="w-full max-w-sm bg-slate-900 border-4 border-slate-700 rounded-lg p-4 flex flex-col items-center relative overflow-hidden">
						...
						{scanState === "idle" ? (
							...
							<span className="text-[9px] mt-4 text-gray-500 font-mono animate-pulse">
								DO NOT STEP OFF PAD
							</span>
						)}
					</div>
