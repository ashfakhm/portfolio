...
import { CrewmateColor } from './CrewmateSprite';
...
export default function TaskModal({ room, onClose, playerColor }: TaskModalProps) {
  ...
  const getTaskContent = () => {
    switch (room) {
      case 'cafeteria':
        return <CafeteriaTask onComplete={() => setTaskCompleted(true)} playerColor={playerColor} />;
      case 'reactor':
        return <ReactorTask onComplete={() => setTaskCompleted(true)} isCompleted={taskCompleted} />;
      case 'admin':
        return <AdminTask onComplete={() => setTaskCompleted(true)} isCompleted={taskCompleted} />;
      case 'comms':
        return <CommsTask onComplete={() => setTaskCompleted(true)} isCompleted={taskCompleted} />;
      case 'medbay':
        return <MedbayTask onComplete={() => setTaskCompleted(true)} isCompleted={taskCompleted} playerColor={playerColor} />;
      case 'security':
        return <SecurityTask onComplete={() => setTaskCompleted(true)} />;
      case 'emergency':
        return <EmergencyTask onComplete={() => setTaskCompleted(true)} playerColor={playerColor} />;
      case 'storage':
        return <StorageTask onComplete={() => setTaskCompleted(true)} isCompleted={taskCompleted} />;
      case 'weapons':
        return <WeaponsTask onComplete={() => setTaskCompleted(true)} isCompleted={taskCompleted} />;
      case 'electrical':
        return <ElectricalTask onComplete={() => setTaskCompleted(true)} isCompleted={taskCompleted} />;
      case 'navigation':
        return <NavigationTask onComplete={() => setTaskCompleted(true)} isCompleted={taskCompleted} />;
      case 'shields':
        return <ShieldsTask onComplete={() => setTaskCompleted(true)} isCompleted={taskCompleted} />;
      default:
        return null;
    }
  };
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto flex flex-col font-mono relative bg-transparent rounded-[20px] m-4 overflow-hidden border border-white/5">
          ...
          {!taskCompleted && room !== 'security' && room !== 'cafeteria' && room !== 'emergency' && (
            <button
