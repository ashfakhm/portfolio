...
const build3DHat = (hatGroup: THREE.Group, type: CrewmateHat) => {
  clearGroup(hatGroup);
...
export default function ThreeCrewmate({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn(
        "ThreeCrewmate: WebGL is not supported, crewmate will not render in 3D:",
        err,
      );
      return;
    }
    ...
    return () => {
      ...
      const disposeGroup = (group: THREE.Group | THREE.Object3D) => {
      ...
      disposeGroup(crewmateGroup);

      bodyGeom.dispose();
      topCapGeom.dispose();
      bottomCapGeom.dispose();
      backpackGeom.dispose();
      visorGeom.dispose();
      visorRimGeom.dispose();
      legGeom.dispose();
