...
export default function ThreeCrewmate({ color, hat, size = 150 }: ThreeCrewmateProps) {
  ...
  const getColorHex = (col: CrewmateColor): number => {
    switch (col) {
      case 'red': return 0xC51111;
      case 'cyan': return 0x38FEDE;
      case 'lime': return 0x50F01E;
      case 'pink': return 0xED54BA;
      case 'yellow': return 0xF5F033;
      case 'orange': return 0xF07D0D;
      case 'purple': return 0x6B2FBC;
      case 'blue': return 0x132ED1;
      case 'white': return 0xD6E0F0;
      case 'black': return 0x2C3035;
      default: return 0xC51111;
    }
  };
  ...
  useEffect(() => {
    ...
    const topCapGeom = new THREE.SphereGeometry(1.6, 24, 16, 0, Math.PI * 2, 0, Math.PI / 2);
    const topCap = new THREE.Mesh(topCapGeom, bodyMat);
    topCap.position.y = 1.3;
    topCap.castShadow = true;
    bodyGroup.add(topCap);

    const bottomCapGeom = new THREE.SphereGeometry(1.6, 24, 16, 0, Math.PI * 2, Math.PI / 2, Math.PI / 2);
    ...
    const build3DHat = (type: CrewmateHat) => {
      ...
      switch (type) {
        case 'crown': {
          ...
          const crownRingGeom = new THREE.CylinderGeometry(0.8, 0.9, 0.5, 12, 1, true);
          const goldMat = new THREE.MeshPhongMaterial({ color: 0xf1c40f, shininess: 80, specular: 0xffffff });
          ...
          for (let i = 0; i < 5; i++) {
            ...
            spike.position.set(Math.cos(angle) * 0.82, 0.2, Math.sin(angle) * 0.82);
        ...
        case 'plant': {
          ...
          const leafMat = new THREE.MeshPhongMaterial({ color: 0x4cd137, shininess: 30 });
        ...
        case 'egg': {
          ...
          const yolkMat = new THREE.MeshPhongMaterial({ color: 0xffbf00, shininess: 60 });
        ...
        case 'pompom': {
          ...
          const balloonMat = new THREE.MeshPhongMaterial({ color: 0xe74c3c, shininess: 100, specular: 0xffffff });
