...
export default function ThreeBackground({ playerMoving = false, playerDirection = { x: 0, y: 0 } }: ThreeBackgroundProps) {
  ...
  useEffect(() => {
    ...
    contextRef.current = { scene, camera, renderer, stars, spaceStation, nebula };
    ...
    const animate = () => {
      ...
      const currentSpeed = playerMoving ? starSpeed * 4 : starSpeed;
      const dirX = playerMoving ? playerDirection.x * 2.5 : 0;
      const dirY = playerMoving ? playerDirection.y * 2.5 : 0;
  ...
  // Update speed variables on prop change
  useEffect(() => {
    // We already query this dynamically in our requestAnimationFrame loop
