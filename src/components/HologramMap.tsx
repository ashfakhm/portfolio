...
export default function HologramMap({
  ...
  return (
    <div className="absolute inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm select-none animate-fadeIn">
      <div className="w-full max-w-5xl bg-[#141926] border-[12px] border-[#303d52] rounded-[32px] p-5 shadow-2xl font-mono flex flex-col justify-between relative" style={{ height: '88vh' }}>
        
        {/* Signature Red Exit Button */}
        <button 
          onClick={() => { onClose(); synthSFX.playBeep(); }}
        ...
        <div className="w-full flex items-center justify-between border-b-4 border-black pb-2.5 mb-1.5 bg-[#0e121d] px-4 py-2 rounded-xl">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-[10px] md:text-xs font-extrabold text-[#e2e8f0] tracking-widest uppercase" style={{ fontFamily: '"Press Start 2P"' }}>
              MAP: THE SKELD
            </h3>
          </div>
        ...
        <div className="w-full flex-1 bg-[#0b0e17] border-[6px] border-black rounded-2xl relative overflow-hidden flex items-center justify-center p-2">
          ...
          <div className="relative w-full h-full max-w-[840px] max-h-[560px] flex items-center justify-center">
            ...
            >
              ...
              {WALKABLE_REGIONS.filter(region => !region.name).map((hall, idx) => (
                <rect
                  key={`hall-${idx}`}
                  x={hall.x}
                  y={hall.y}
              ...
              {Object.entries({
                ...
                const roomInfo = WALKABLE_REGIONS.find(r => r.name === roomId);
                if (!roomInfo) return null;

                return (
                  <g key={`label-${roomId}`} className="pointer-events-none select-none">
                    {/* Drop shadow label */}
                    <text
                      x={label.x}
                      y={label.y + 2}
              ...
              {FLOATING_VENTS.map((v) => (
                ...
                >
                  ...
                  <rect x={-8} y={-8} width={16} height={16} fill="#4b5563" stroke="#000000" strokeWidth={2.5} rx={2} />
                  <line x1={-5} y1={-4} x2={5} y2={-4} stroke="#000000" strokeWidth={2} />
              ...
              {Object.entries(completedTasks).map(([roomId, isDone]) => {
                ...
                return (
                  ...
                  >
                    ...
                    <circle r={22} className="fill-yellow-500/10 stroke-yellow-500/25 animate-ping" />
          ...
          <div className="absolute bottom-4 left-4 bg-black/85 border-2 border-slate-700/80 px-3.5 py-2 rounded-xl text-[9px] text-[#22c55e] uppercase tracking-wider flex items-center gap-2">
            ...
            <span>Nav GPS: Real-time Coordinates Tracking ({playerPos.x}, {playerPos.y})</span>
          </div>
        </div>
        ...
        <div className="text-center text-[9px] md:text-[10px] text-slate-400 font-bold bg-[#0e121d] py-2.5 rounded-xl border-t-2 border-black tracking-wide uppercase">
          ⚡ CLICK ANY WORK ROOM ON THE MONITOR TO AUTOMATICALLY ENGAGE INTUITIVE AUTOPILOT NAVIGATION ⚡
        </div>
      </div>
    </div>
  );
