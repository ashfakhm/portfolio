...
export function drawShip(
	...
	WALKABLE_REGIONS.forEach((reg) => {
		if (!reg.name) {
			...
			for (let lx = reg.x + 20; lx < reg.x + reg.w; lx += 24) {
				ctx.beginPath();
				ctx.moveTo(lx, reg.y);
				ctx.lineTo(lx, reg.y + reg.h);
				ctx.stroke();
			}
			for (let ly = reg.y + 20; ly < reg.y + reg.h; ly += 24) {
				ctx.beginPath();
				ctx.moveTo(reg.x, ly);
				ctx.lineTo(reg.x + reg.w, ly);
				ctx.stroke();
			}
	...
	Object.values(SPACESHIP_ROOMS).forEach((room) => {
		...
		const isCompleted =
			completedTasks[room.id] !== undefined ? completedTasks[room.id] : true;
		if (isCompleted) {
		...
		if (room.id === "cafeteria") {
			...
			ctx.beginPath();
			ctx.arc(epX, epY, 34, 0, Math.PI * 2);
			ctx.fill();
			ctx.strokeStyle = "#000000";
			ctx.lineWidth = 3;
			ctx.stroke();
			...
			ctx.fillStyle = "#ebaf17";
			ctx.beginPath();
			ctx.arc(epX, epY, 26, 0, Math.PI * 2);
			ctx.fill();
			ctx.stroke();
			...
			ctx.beginPath();
			ctx.arc(epX, epY, 15, 0, Math.PI * 2);
			ctx.fill();
			ctx.stroke();
			...
			tablesList.forEach((tb) => {
				...
				ctx.roundRect
					...
					: ctx.rect(tb.x - tWidth / 2, tb.y - tHeight / 2, tWidth, tHeight);
				ctx.fill();
				ctx.strokeStyle = "#000000";
				ctx.lineWidth = 2.5;
				ctx.stroke();
				...
				ctx.beginPath();
				ctx.arc(tb.x + 10, tb.y, 4, 0, Math.PI * 2);
				ctx.fill();
				ctx.stroke();
				ctx.fillStyle = "#3498db";
				ctx.beginPath();
				ctx.arc(tb.x + 10, tb.y, 2, 0, Math.PI * 2);
				ctx.fill();
				...
				[-20, 0, 20].forEach((ox) => {
					// Top stools
					ctx.beginPath();
					ctx.arc(tb.x + ox, tb.y - tHeight / 2 - 8, 5, 0, Math.PI * 2);
					ctx.fill();
			...
			ctx.beginPath();
			ctx.ellipse(padX, padY, 18, 12, 0, 0, Math.PI * 2);
			ctx.fill();
			...
			ctx.lineTo(
				padX + Math.cos(sweepAngle) * 18,
				padY + Math.sin(sweepAngle) * 12,
			);
			ctx.stroke();
			...
			ctx.fillStyle = "#4c5e5c";
			ctx.beginPath();
			ctx.ellipse(585, 405, 50, 35, 0, 0, Math.PI * 2);
			ctx.fill();
			ctx.strokeStyle = "#000000";
			ctx.lineWidth = 3;
			ctx.stroke();
			...
			ctx.beginPath();
			ctx.ellipse(585, 405, 42, 28, 0, 0, Math.PI * 2);
			ctx.fill();
			...
			for (let i = -30; i <= 30; i += 15) {
				ctx.beginPath();
				ctx.moveTo(585 + i, 380);
				ctx.lineTo(585 + i, 430);
				ctx.stroke();
				ctx.beginPath();
				ctx.moveTo(545, 405 + i / 1.5);
				ctx.lineTo(625, 405 + i / 1.5);
				ctx.stroke();
			}
			...
			ctx.lineWidth = 3;
			ctx.beginPath();
			ctx.moveTo(room.cx - 25, room.cy - 20);
			ctx.lineTo(room.cx + 25, room.cy + 20);
			ctx.stroke();
			ctx.beginPath();
			ctx.moveTo(room.cx - 25, room.cy);
			ctx.lineTo(room.cx + 10, room.cy + 20);
			ctx.stroke();
			...
			const barrelsList = [
				{ x: room.bounds.maxX - 15, y: room.bounds.maxY - 15 },
				{ x: room.bounds.maxX - 15, y: room.bounds.maxY - 35 },
				{ x: room.bounds.maxX - 30, y: room.bounds.maxY - 25 },
			];
			barrelsList.forEach((br) => {
				ctx.fillStyle = "#27ae60";
				ctx.beginPath();
				ctx.ellipse(br.x, br.y, 8, 10, 0, 0, Math.PI * 2);
				ctx.fill();
				ctx.strokeStyle = "#000000";
				ctx.lineWidth = 2;
				ctx.beginPath();
			...
			for (let ix = 0; ix < 2; ix++) {
				...
				ctx.fillStyle =
					sT > 0.4 ? "#2ecc71" : sT > -0.4 ? "#f1c40f" : "#e74c3c";
			...
			ctx.strokeStyle = "#000";
			ctx.lineWidth = 3;
			ctx.strokeRect(room.cx - 24, room.cy - 15, 48, 30);
			...
			ctx.beginPath();
			ctx.ellipse(room.cx - 35, room.cy, 12, 18, 0, 0, Math.PI * 2);
			ctx.fill();
			ctx.strokeStyle = "#000";
			ctx.lineWidth = 2;
			ctx.stroke();
			...
			ctx.fillStyle = "#2ecc71";
			ctx.beginPath();
			ctx.arc(672, 285, 4, 0, Math.PI * 2);
			ctx.fill();
			ctx.beginPath();
			ctx.arc(682, 282, 5, 0, Math.PI * 2);
			ctx.fill();
		}

		// 5. Drawing active clickable console coordinate indicators (yellow glossy spheres)
		const isCompletedRoom =
			completedTasks[room.id] !== undefined ? completedTasks[room.id] : true;
		...
		ctx.beginPath();
		ctx.arc(room.cx - 3, room.cy - 3, 3, 0, Math.PI * 2);
		ctx.fill();
